...
function Sidebar({ user, isCollapsed, setCollapsed }) {
    ...
    //to check for overflow is mobile 
    useEffect(() => {
    ...
    return (
        <>
            <motion.div ref={sidebarRef} className={sidebarStyles.sidebarContainer.base} initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1, width: isCollapsed ? '80px' : '256px' }} transition={{ type: "spring", damping: 25 }}>
            ...
            <motion.button onClick={() => setMobileOpen((prev) => !prev)}
                ...
                whileTap={{ scale: 0.95 }}
            >
                {mobileOpen ? <X size={24}></X> : <Menu size={24}></Menu>}
                        ...
                        >
                            <div className="relative h-full flex flex-col">
                                ...
                                <div className={sidebarStyles.mobileFooter}>
                                    <Link onClick={() => setMobileOpen(false)} to="http://hexagondigitalservices.com/contact" className={sidebarStyles.mobileFooterLink}>
